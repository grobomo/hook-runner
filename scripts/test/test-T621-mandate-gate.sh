...
# 2. Has WORKFLOW tag
if grep -q '// WORKFLOW: wsl' "$MOD"; then
  pass "WORKFLOW: wsl tag present"
else
...
# 6. Returns null when no mandate.json exists
...
")
if [ "$RESULT" = "null" ]; then
  pass "No mandate.json → pass (null)"
else
  fail "No mandate.json → expected null, got: $RESULT"
fi
...
cat > "$TMPDIR/.claude/hooks/mandate.json" <<'EOF'
{
...
# 8. Block reason contains MANDATE prefix
RESULT=$(HOME="$TMPDIR" node -e "
  var fs = require('fs');
  // Reset seen
  var m = JSON.parse(fs.readFileSync('$TMPDIR/.claude/hooks/mandate.json', 'utf-8'));
  m.seen = false;
  fs.writeFileSync('$TMPDIR/.claude/hooks/mandate.json', JSON.stringify(m));
  delete require.cache[require.resolve('$MOD')];
  var gate = require('$MOD');
  var r = gate({ tool_name: 'Bash', tool_input: { command: 'ls' } });
  console.log(r && r.reason && r.reason.indexOf('MANDATE') !== -1 ? 'yes' : 'no');
")
if [ "$RESULT" = "yes" ]; then
  pass "Block reason contains MANDATE prefix"
else
  fail "Block reason missing MANDATE prefix"
fi
...
# 11. Block reason contains source rule name
RESULT=$(HOME="$TMPDIR" node -e "
  var fs = require('fs');
  var m = JSON.parse(fs.readFileSync('$TMPDIR/.claude/hooks/mandate.json', 'utf-8'));
  m.seen = false;
  ...
  var gate = require('$MOD');
  var r = gate({ tool_name: 'Bash', tool_input: { command: 'ls' } });
  console.log(r && r.reason && r.reason.indexOf('incomplete-todos') !== -1 ? 'yes' : 'no');
")
if [ "$RESULT" = "yes" ]; then
  pass "Block reason contains source rule"
else
  fail "Block reason missing source rule"
fi
...
")
if [ "$RESULT" = "true" ]; then
  pass "mandate.json seen=true after first block"
else
  fail "mandate.json seen not updated, got: $RESULT"
fi
...
if [ ! -f "$TMPDIR/.claude/hooks/mandate.json" ]; then
  pass "Expired mandate.json is deleted"
else
  fail "Expired mandate.json should be deleted"
fi
...
# 17. Corrupt JSON in mandate.json → pass
echo "not json" > "$TMPDIR/.claude/hooks/mandate.json"
...
")
if [ "$RESULT" = "null" ]; then
  pass "Corrupt mandate.json → pass (null)"
else
  fail "Corrupt mandate.json → expected null, got: $RESULT"
fi
...
# 21. auto-continue-gate.js writes mandate.json on CONTINUE
if grep -q 'writeFileSync(MANDATE_PATH' "$REPO_DIR/modules/Stop/auto-continue-gate.js"; then
  pass "auto-continue-gate.js writes mandate.json on CONTINUE"
else
...
if grep -q 'unlinkSync(MANDATE_PATH' "$REPO_DIR/modules/Stop/auto-continue-gate.js"; then
  pass "auto-continue-gate.js clears mandate.json on DONE"
else
