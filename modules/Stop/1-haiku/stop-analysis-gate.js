...
//
// INCIDENT HISTORY:
//   2026-05-20: T667 redesign — gate now ALWAYS returns block (even DONE).
//   Opus reads the reason text to decide whether to stop. TUI always shows.
//
// ┌─────────────────────────────────────────────────────────────────────────┐
// │ STOP ANALYSIS GATE — Haiku evaluates whether session should end         │
// │                                                                         │
// │ On every Stop event: calls Haiku with session context + rules,          │
// │ writes reasoning to stop-analysis.md, returns block decision.           │
// │                                                                         │
// │ T667: ALWAYS returns block — DONE or CONTINUE — so TUI shows every     │
// │ decision. Opus reads "DONE" in the reason and stops. Never invisible.   │
// │                                                                         │
// │ Install: symlink or copy into ~/.claude/hooks/run-modules/Stop/1-haiku/ │
...
module.exports = function(input) {
  ...
  // Read rules — prefer directory (T758 modular) over monolith
  var rules = "";
  ...
  try {
    if (fs.existsSync(RULES_DIR)) {
      ...
      for (var ri = 0; ri < ruleFiles.length; ri++) {
        var rc = fs.readFileSync(path.join(RULES_DIR, ruleFiles[ri]), "utf-8");
        // Strip comments, keep name/check/action lines
        var lines = rc.split("\n").filter(function(l) { return !l.startsWith("#") && l.trim(); });
        ruleTexts.push(lines.join("\n"));
      }
      rules = "rules:\n" + ruleTexts.map(function(t) { return "  - " + t.replace(/\n/g, "\n    "); }).join("\n");
    } else {
  ...
  if (decision === "continue") {
    ...
    return {
      decision: "block",
      reason: "SELF-CHECK [" + (ruleTriggered || "stop-analysis") + "]: CONTINUE — " + reason
    };
  }

  return {
    decision: "block",
    reason: "SELF-CHECK [" + (ruleTriggered || "stop-analysis") + "]: DONE — " + reason + "\nNo further action needed. You may stop."
  };
};
