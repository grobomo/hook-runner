...
- [ ] T641: Fix stop hook — auto-continue.js in BLOCKING_MODULES preempts Haiku stop-analysis-gate.js. Haiku never gets to evaluate. Options: (a) remove auto-continue from BLOCKING_MODULES, let Haiku decide, (b) run Haiku first with short timeout, fall back to auto-continue. Currently the stop hook always shows generic "DO NOT STOP" — Haiku analysis goes to background and is invisible in TUI.
- [ ] T642: Fix settings-watchdog-gate.js corruption — 4 spurious `return null;` lines (84, 90, 98, 139) were removed via python patch but should be properly fixed in source. Lines were: `".trim()});` + bare `return null;` injected by a previous bad edit. Gate now works (verified 7/7 attack vectors) but the fix was out-of-band.
...
- [ ] T639: Standardize module meta parsing — replace separate parseToolTags/parseWorkflowTags/isBlocking with single parseModuleMeta() that returns all tags in one pass. Central registry of known tags. Add BLOCKING tag for Stop modules (replaces hardcoded list in run-stop.js).
