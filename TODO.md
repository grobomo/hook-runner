...
- [ ] T615: Fix stop-analysis-gate.js not showing in TUI — missing `// BLOCKING: true` tag in header. Without it, run-stop.js sends it to background worker and Haiku's analysis never reaches stdout. Add `// BLOCKING: true` after the `// WORKFLOW: wsl` line. Also: run-stop.js hardcodes `BLOCKING_MODULES = ["auto-continue", "never-give-up"]` as legacy fallback — stop-analysis-gate is not in this list. The `isBlocking()` check from load-modules.js should catch the tag, but the tag is missing. One-line fix: add `// BLOCKING: true` to line 3 of stop-analysis-gate.js.
