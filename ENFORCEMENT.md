...
|----------|-------|-------------|
| Active gates | 2 | Blocked 10+ times — proven enforcement |
| Low-activity gates | 11 | Blocked 1-10 times — edge cases |
| Preventive gates | 42 | Loaded 100+ times, never blocked — deterrent |
| Unused/untested gates | 7 | Candidates for removal |
...
**Blocks:** 28 / 590 invocations
**Last block:** 2026-04-11T12:56:19.548Z

### remote-tracking-gate
**Why:** Commits on untracked branches were invisible on mobile.
**Blocks:** 13 / 621 invocations
**Last block:** 2026-04-11T12:49:42.946Z

## Low-Activity Gates

- **archive-not-delete** — Claude deleted files that turned out to be needed later. Block destructive delete commands. Always archive, never delete. Returns null to pass, {decision:"block", reason:"..."} to block. (1 blocks)
- **branch-pr-gate** — Claude committed directly to main, bypassing review. (9 blocks)
- **commit-counter-gate** — Claude makes 20+ file changes without committing, then context resets and all work is lost or untraceable. User tracks progress via GitHub Mobile. Every 5 edits, force a commit so there's a git trail. (10 blocks)
- **commit-quality-gate** — Generic commit messages like "fix" or "update" make git history useless. When debugging E2E failures across 10+ deploy cycles, you need to know what each commit actually changed and why. Bad messages waste 10+ minutes per cycle. (1 blocks)
- **enforcement-gate** — Code edits in repos without TODO.md or with dirty trees caused lost work. Enforcement gate: git repo, clean tree, TODO.md required before Edit/Write (1 blocks)
- **gh-auto-gate** — `gh auth switch` is broken with EMU accounts — the API still uses the EMU token even after switching. Raw `gh` and `git push` commands silently use the wrong account, causing 403s or pushing to the wrong org. gh_auto reads .github/publish.json and sets GH_TOKEN correctly every time. Tagged security (never disable) instead of shtd. (7 blocks)
- **git-destructive-guard** — Claude ran `git reset --hard` and `git checkout .` to "clean up" working trees, destroying uncommitted work. These ops are rarely the right solution — investigate root cause instead. Only rebase was gated (git-rebase-safety); other destructive ops were unguarded. (1 blocks)
...
- **force-push-gate** — Force-pushing to main/master can destroy shared history and others' work. There is no undo for a force-push that overwrites remote commits.
...
- **hook-editing-gate** — A rogue Claude tab silently weakened spec-gate.js by removing Bash from the gated tools list. No audit trail, no alert. Any session could modify hooks to bypass its own enforcement. This gate now locks hook editing to the hook-runner project only, with static weakening detection for all changes. T339: Project-locked + weakening detection + self-edit protection.
- **hook-system-reminder** — Claude repeatedly tries to create .claude/rules/ files despite being told dozens of times across dozens of sessions. This hook fires when Claude tries to WRITE or EDIT anything in ~/.claude/ to remind it how the system works. It's a soft block — Claude can proceed after reading the reminder.
